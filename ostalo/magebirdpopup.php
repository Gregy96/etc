...
if(isset($_REQUEST['changePubDir'])){
  $class->model->changePubDir();
}  
  
$action = isset($_REQUEST['action']) ? $_REQUEST['action'] : 'show'; 
switch($action){    
  case "show": 
    require_once realpath(__DIR__) . '/../lib/internal/magebird/popup/Mobile_Detect.php';
    require_once realpath(__DIR__) . '/../lib/internal/magebird/popup/MaxMind/Db/Reader2.php';
    require_once realpath(__DIR__) . '/../lib/internal/magebird/popup/MaxMind/Db/Reader/Decoder2.php';
    require_once realpath(__DIR__) . '/../lib/internal/magebird/popup/MaxMind/Db/Reader/InvalidDatabaseException2.php';
    require_once realpath(__DIR__) . '/../lib/internal/magebird/popup/MaxMind/Db/Reader/Metadata2.php';
    require_once realpath(__DIR__) . '/../lib/internal/magebird/popup/MaxMind/Db/Reader/Util2.php';    
    $class->model->addNewView();
    $class->showAction();    
    break;
  case "stats":
    $class->statsAction();    
  ...
  case "popupCartsCount":
    $class->model->uniqueViewStats($this->helper->getParam('popupId'));
    break;   
  default:
    $class->statsAction();
}
